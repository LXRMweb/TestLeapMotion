 ...
 * */
class LXRMListener extends Listener {
    ...
    public void onFrame(Controller controller) {
    	...
    	String path=null;
    	try {
    		path=frameFileOperation.writeFile(10);//每次onFrame()时，调用一次该函数，读取最新一帧以及历史中9帧数据，序列化后写到相应文件中
		} catch (IOException e) {
...
class TestLeapMotionFrameFileOperation {
    public static void main(String[] args) throws IOException {
    	System.out.println("执行TestLeapMotionFrameFileOperation类的main");
        // Create a sample listener and controller
        LXRMListener listener = new LXRMListener();
		...
		*/
        while(true){
        	for(int i=0;i<4000000;i++){//等若干时间，使得数据稳定之后再写Frame数据到文件
        	}
        	//break;
        }
